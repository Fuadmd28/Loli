        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit Kamu Habis, Beli Dengan Cara *${usedPrefix}buy limit, Atau membeli dengan ketik .sewa*`, m)
...
export async function deleteUpdate(message) {
    try {
        ...
        if (chat.delete || !global.db.data.config['delete_enabled'])
            return
...
let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
  let nyaww = 'https://i.ibb.co/Fznhq78/20230620-115751.jpg'
  let fotodaftar = 'https://telegra.ph/file/60990121dc5bcd2a376a1.jpg'
let msg = {
        onlyprem: 'Untuk menggunakan fitur ini di private chat anda bisa membeli nya ketik *.premium* atau lebih murah nya *.sewa*',
        owner: 'Ngapain Kak?, Fitur Ini Khusus Ownerku',
        mods: 'Fitur Ini Khusus Moderator',
        private: 'Fitur Ini Khusus Pribadi Chat',
        admin: 'Fitur Ini Khusus Admin Group 🥴',
        premium: 'Fitur Ini Khusus Premium User, Jika Mau Membeli Ketik *.premium*',
        group: 'Fitur Ini Hanya Bisa Digunakan Di Grup, Jika Mau Membeli Ketik *.sewa*',       botAdmin: 'Jadikan Lofi Sebagai Admin Terlebih Dahulu Agar Bisa Menggunakan Fitur Ini',
